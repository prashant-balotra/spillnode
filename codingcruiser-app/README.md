...
A full-stack blog platform for the **CodingCruiser** YouTube channel — Java, Spring Boot, Angular, Next.js, full-stack & cloud tutorials.
...
- MySQL → `localhost:3306` (user: `cruiser`, db: `codingcruiser`)

Stop: `docker compose down` &nbsp; · &nbsp; Wipe DB: `docker compose down -v`

The admin account `admin@codingcruiser.com` / `Admin@123` and **5 sample posts** are seeded automatically on first boot.
...
Data resets on restart. H2 console available at http://localhost:8080/h2-console (JDBC URL `jdbc:h2:mem:codingcruiser`, user `sa`, blank password).
...
│   ├── pom.xml
│   └── src/main/
│       ├── java/com/codingcruiser/
│       │   ├── CodingCruiserApplication.java
...
Built with ❤️ for CodingCruiser viewers.
